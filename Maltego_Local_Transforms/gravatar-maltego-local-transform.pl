...
$email_address =~ s/\s+$//;
print STDERR
  "Value of \$email_address after whitespace trim is \"$email_address\"\n";

# Force all characters of the e-mail address to lower-case
$email_address = lc($email_address);
print STDERR
"Value of \$email_address after conversion to lowercase is \"$email_address\"\n";

# Calculate the MD5 hash of the e-mail address
my $md5_of_email_address = md5_hex($email_address);
print STDERR
"Value of \$md5_of_email_address after conversion to lowercase is \"$md5_of_email_address\"\n";
...
print("<UIMessages>\n");
print(
"		<UIMessage MessageType=\"Inform\">To Gravatar Affilation - Local Transform v$VERSION</UIMessage>\n"
);
print("</UIMessages>\n");
...
print("\t</Entity>\n");
print(
"\t\t<Entity Type=\"cmlh.affiliation.gravatar\"><Value>$gravatar_url</Value>\n"
);
print("\t\t<AdditionalFields>\n");
print(
"\t\t\t<Field Name=\"properties.affiliation.gravatar\">$gravatar_url</Field>\n"
);
print("\t\t</AdditionalFields>\n");
print("\t</Entity>\n");
print("\t</Entities>\n");
